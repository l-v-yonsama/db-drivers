  ...
  describe('asyncGetResouces', () => {
    ...
    describe('specify resource filters', () => {
      ...
      it('prefix', async () => {
        ...
        const schemaRes = testDbRes.getSchema({ isDefault: true });
        ...
        for (const c of [1, 3]) {
          for (const d of [1, 3]) {
            ...
            const tableRes = schemaRes.getChildByName(tableName);
            expect(tableRes).toBeUndefined();
          }
        }
      });
