...
describe('cfn', () => {
  describe('viewpoint / auxiliaryTreatment', () => {
    ...
    it("MergeIntoLabel (default): folds each auxiliary resource onto its focus neighbor's label instead of giving it a node/edge of its own", () => {
      ...
      // Each auxiliary resource's id survives as a merged annotation on the focus resource(s) it had an edge with, instead - GreetingFunction has one from GreetingFunction->Role and one from Permission->GreetingFunction, so it picks up both.
      ...
      // GreetingApiDeployment depended on both GreetingMethod and GreetingApi - both focus neighbors pick up its id.
      ...
      // No arrow touches an auxiliary resource - the four focus-to-focus edges survive, including the Fn::Sub SourceArn reference from the invoke permission to the API.
    ...
    it("SeparateGroup: keeps every auxiliary resource as its own node in a 'Supporting' group, with no edges touching it", () => {
      ...
      // Auxiliary resources render with their normal icon/label, just relocated - no merged annotation text, since nothing needed folding onto anyone.
      expect(diagram).toContain(
      ...
      // Only the four focus-to-focus edges remain - none of the auxiliary resources above get an edge, per the explicit "no arrows for auxiliary elements" requirement.
      expect(diagram.match(/(?:-->|-.->|==>)/g)).toHaveLength(4);
    ...
    it("CloudFormationView shows every resource as focus regardless of auxiliaryTreatment - matches today's unfiltered output", () => {
      ...
      // auxiliaryTreatment is meaningless once nothing is auxiliary - same output no matter which one is passed alongside CloudFormationView.
    ...
    it('classification genuinely depends on the viewpoint, not just "IAM-ish things are always auxiliary"', () => {
      // Infrastructure View's list explicitly includes IAM Role - and does *not* include Lambda - so the focus/auxiliary split flips relative to ApplicationView for this same fixture.
